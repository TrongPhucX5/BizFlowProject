...
export default function CustomerPage() {
  ...
  const pageData = useMemo(() => {
    ...
    return {
      ...
      last:
        raw?.last ??
        (raw?.page ? raw.page.number + 1 >= raw.page.totalPages : true),
  ...
  const chartStats = useMemo(() => {
    const top5 = [...customerList]
      .sort(
        (a, b) => Number(b.totalPurchaseAmount) - Number(a.totalPurchaseAmount),
      )
      .slice(0, 5)
      .map((c) => ({
  ...
  const stats = useMemo(() => {
    ...
    const pageDebt = customerList.reduce(
      (sum, c) => sum + (Number(c.totalDebt) || 0),
      0,
    );
    const pageSales = customerList.reduce(
  ...
  const handleExportTT88Debt = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `so-cong-no-TT88-${exportFrom}-${exportTo}.xlsx`,
      );
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        ...
        <div className="flex flex-wrap gap-3 w-full lg:w-auto">
          <div className="relative flex-1 md:w-80 group">
            <Search
              className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 transition-colors ${inputValue ? "text-indigo-600" : "text-slate-400"}`}
            />
            <Input
              ...
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="hover:bg-transparent border-b border-slate-100">
                    <TableHead className="font-bold py-5 px-6 text-slate-600 uppercase text-[11px]">
                      Thông tin khách hàng
                    </TableHead>
                    <TableHead className="font-bold text-center text-slate-600 uppercase text-[11px]">
                      Loại
                    </TableHead>
                    <TableHead className="font-bold text-right text-slate-600 uppercase text-[11px]">
                      Tổng mua
                    </TableHead>
                    <TableHead className="font-bold text-right text-slate-600 uppercase text-[11px]">
                      Công nợ
                ...
                <TableBody>
                  {isError ? (
                    <TableRow>
                      <TableCell
                        colSpan={6}
                        className="text-center py-20 text-rose-500 font-bold"
                      >
                        Lỗi kết nối Server!
                      </TableCell>
                    </TableRow>
                    ...
                    customerList.map((item) => (
                      <TableRow
                        key={item.id}
                        className="group hover:bg-indigo-50/30 transition-colors border-b border-slate-50"
                      >
                        ...
                        <TableCell className="text-center">
                          <Badge
                            className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase border-none ${item.type === "WHOLESALE" ? "bg-slate-900 text-white" : "bg-indigo-100 text-indigo-700"}`}
                          >
                        ...
                        <TableCell className="text-right font-black text-sm text-slate-900">
                          {(
                            Number(item.totalPurchaseAmount) || 0
                          ).toLocaleString()}
                          đ
                        </TableCell>
                        ...
                        <TableCell className="px-4 text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent
                              align="end"
                              className="w-48 p-2 rounded-xl border-none shadow-2xl"
                            >
                              <DropdownMenuItem
                              ...
                              <DropdownMenuItem
                                className="cursor-pointer font-bold text-xs text-rose-600"
                                onClick={() => {
              ...
              <div className="p-6 border-t border-slate-50 flex items-center justify-between bg-slate-50/30">
                <div className="flex flex-col">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                    Trang {page + 1} / {stats.totalPages}
                  </p>
                  <p className="text-[10px] text-slate-400 font-medium italic">
                    Hiển thị {stats.currentPageCount} / {stats.totalCount}
                  </p>
                </div>
          ...
          <Card className="border-none shadow-xl bg-white rounded-3xl p-8">
            <CardHeader className="p-0 mb-8 flex flex-row items-center justify-between">
              <CardTitle className="text-sm font-black uppercase text-slate-400 tracking-widest flex items-center gap-2">
                <BarChart3 className="h-4 w-4 text-indigo-600" /> Top 5 Mua hàng
                & Nợ (Trang này)
              </CardTitle>
            </CardHeader>
            ...
            <div className="bg-white/10 rounded-2xl p-4 text-[11px] leading-relaxed italic border border-white/10">
              {stats.pageSales > 0 && stats.pageDebt > stats.pageSales * 0.3 ? (
                <p className="text-rose-100 font-bold">
                  ⚠️ Cảnh báo: Tỷ lệ nợ cao (
                  {((stats.pageDebt / stats.pageSales) * 100).toFixed(0)}%). Cần
                  rà soát nợ quá hạn.
                </p>
              ) : (
                <p>
                  ✅ Chỉ số dòng tiền ổn định. Các khoản nợ nằm trong tầm kiểm
                  soát.
                </p>
              )}
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-sm font-black ${stats.pageSales > 0 && stats.pageDebt / stats.pageSales > 0.25 ? "text-rose-600" : "text-emerald-600"}`}
                >
                  {stats.pageSales > 0
                    ? ((stats.pageDebt / stats.pageSales) * 100).toFixed(1)
                    : 0}
                  %
                </span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, stats.pageSales > 0 ? (stats.pageDebt / stats.pageSales) * 100 : 0)}%`,
                  }}
                />
              </div>
            </div>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent className="rounded-3xl p-8 max-w-md border-none shadow-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="text-2xl font-black uppercase text-center">
              Xác nhận xóa?
            </AlertDialogTitle>
            <AlertDialogDescription className="text-center font-medium py-2">
              Mọi dữ liệu của đối tác{" "}
          ...
          <AlertDialogFooter className="mt-8 flex gap-3 sm:justify-center">
            <AlertDialogCancel className="flex-1 rounded-xl font-bold h-12">
              HỦY
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() =>
    ...
    <Card className="border-none shadow-lg bg-white rounded-2xl overflow-hidden hover:shadow-indigo-100 transition-all group cursor-default">
      <CardContent className="p-6 flex justify-between items-center">
        <div>
          ...
          <h3
            className={`text-2xl font-black tracking-tighter ${alert ? "text-rose-600" : "text-slate-900"}`}
          >
            {value}
          </h3>
        </div>
        <div className="p-3.5 bg-slate-50 rounded-2xl group-hover:bg-indigo-50 transition-colors shadow-sm">
          {icon}
        </div>
      </CardContent>
    </Card>
