...
import { Plus, TrendingUp, PackageSearch, Users, AlertTriangle, Mic, Loader2 } from "lucide-react";

// --- Định nghĩa Interfaces dựa trên Backend ---
interface ApiResponse<T> {
    code: number;
    message: string;
    result: T; // Khớp với private T result trong Java
    timestamp: string;
}

interface PageResponse<T> {
    content: T[]; // Dữ liệu thực tế nằm trong content
...
export default function DashboardPage() {
    ...
    const { data: productsRes, isLoading: isLoadingProd } = useQuery<ApiResponse<PageResponse<any>>>({
    ...
    const { data: ordersRes, isLoading: isLoadingOrders } = useQuery<ApiResponse<PageResponse<any>>>({
        queryKey: ["orders-all"],
    ...
    if (isChecking || isLoadingProd || isLoadingOrders) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-slate-50">
                ...
                <p className="text-slate-500 font-medium">Đang tải dữ liệu BizFlow...</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-6 space-y-8 bg-slate-50 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Tổng quan BizFlow</h1>
                    <p className="text-slate-500 text-sm italic">Hệ thống đồng bộ dữ liệu lúc {new Date().toLocaleTimeString()}</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-2 shadow-sm border-none bg-white">
                    <CardHeader className="flex flex-row items-center justify-between border-b pb-4">
                        <CardTitle className="text-lg font-bold">Cảnh báo tồn kho thấp</CardTitle>
                                ...
                                {stats.lowStockItems.length > 0 ? (
                                    stats.lowStockItems.map((item: any) => (
                                        <TableRow key={item.id}>
                                            ...
                                            <TableCell className="text-red-600 font-bold">{item.stock} {item.unitName}</TableCell>
                                            <TableCell className="text-right">
                                                <Button variant="outline" size="sm" className="text-indigo-600">Nhập hàng</Button>
                                            </TableCell>
                                        </TableRow>
                                    ))
                                ) : (
                                    <TableRow>
                                        <TableCell colSpan={3} className="text-center py-4 text-slate-400">Kho hàng an toàn</TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                        </Table>
...
function StatCard({ title, value, icon, trend, alert = false }: any) {
    return (
        <Card className={`shadow-sm border-none bg-white ${alert ? 'ring-2 ring-red-100' : ''}`}>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-xs font-bold text-slate-400 uppercase tracking-wider">{title}</CardTitle>
                <div className="p-2 bg-slate-50 rounded-lg">{icon}</div>
            </CardHeader>
            <CardContent>
                <div className={`text-2xl font-black ${alert ? 'text-red-600' : 'text-slate-800'}`}>{value}</div>
                <p className="text-[10px] mt-1 text-slate-400 font-medium italic">{trend}</p>
            </CardContent>
        </Card>
    );
}